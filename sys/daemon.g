...
while true
	...
	if (network.interfaces[0].actualIP="0.0.0.0") || (network.interfaces[0].actualIP=null)  || (network.interfaces[0].state!="active")
		...
		if exists(global.NetworkRestartTime)
			if global.NetworkRestartTime < state.upTime
				if fileexists("0:/sys/print_log.txt")
					echo >>"0:/sys/print_log.txt" "WiFi disconnected:  Restart attempted at : " ^ state.upTime
					if state.time != null
						>>"0:/sys/print_log.txt" "WiFi disconnected:  Restart attempted at : " ^ state.time
				echo "start logging"
				...
				set global.NetworkRestartTime = (state.upTime + 120)
		else
			global NetworkRestartTime = (state.upTime + 120)
			if fileexists("0:/sys/print_log.txt")
				echo >>"0:/sys/print_log.txt" "WiFi disconnected:  WiFi module restart schedued for " ^ global.NetworkRestartTime
				if state.time != null
					>>"0:/sys/print_log.txt" "WiFi disconnected:  WiFi module restart schedued for " ^ state.time + 120
	
	if (state.status == "paused")  && (heat.heaters[tools[state.currentTool].heaters[0]].current > heat.coldExtrudeTemperature)
		if exists(global.pausePurgeAmount) && exists(global.pausePurgeActive=true)
			if global.pausePurgeAmount > 0
				G1 E{global.pausePurgeAmount} F2
				M400
