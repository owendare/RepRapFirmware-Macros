...
if !exists(global.PrintSpeed)
	global PrintSpeed = 50 ; global to set the print speed used in slicer - used to adjust PA for speed
else
	set global.PrintSpeed = 50
...
if !exists(global.BedHasSoaked) ; used to determine if bed soaking is required.
	global BedHasSoaked = false 
else
	set global.BedHasSoaked = false 

if !exists(global.pausePurgeActive) ; used to determine if filament should be purged duriing pauses.
	global pausePurgeActive = false 
else
	set global.pausePurgeActive = false 

if !exists(global.pausePurgeAmount) ; used to determine if filament should be purged duriing pauses.
	global pausePurgeAmount = 0.5 
else
	set global.pausePurgeAmount = 0.5

;if !exists(global.NetworkRestartTime)
;	global NetworkRestartTime = state.time + 360 ; allow 3 minutes for network to coonnect before daemon tries to reset 
...
M586 P1 S1                                                  ; disble FTP
M586 P2 S1                                                  ; disable Telnet
M554 P192.168.1.1											; set gateway
M553 P255.255.255.0											; set netmask
M552 S1 P"DARE_EXT"                                         ; enable network
...
M915 H240 X Y S6										; set stall detection for X & Y axis. driver-stall.g will be run.
...
; Axis Limits
M208 X0:218 Y0:285 Z0:200 									; X carriage moves from 0 to 200, Y bed goes from -2 to 200
...
M558 P9 C"^zprobe.in" H6:2 F600:90 T4800 A10 S0.03 R0.5 B1  ; set Z probe type to bltouch and the dive height + speeds - turn off heaters when probing
...
M98 P"0:/sys/retractprobe.g" ; send retract signal
...
; Heaters
M308 S0 P"bedtemp" Y"thermistor" A"Bed"  T100000 B4725 C7.060000e-8 ; configure sensor 0 as thermistor on pin bedtemp connected to heat pad - main bed sensor
M950 H0 C"bedheat" T0:8 Q10                                       ; create bed heater output on bedheat and map it to sensor 0 and set PWM frequency to 10hz
M140 H0 																		; Set bed themp to zero
M143 H0 S130 A0 C0                                          ; set temperature limit for heater 0 to 130C - fault if too high
M308 S8 P"exp.thermistor5" Y"thermistor" A"Bed_Soak"  T100000 B4725 C7.060000e-8   ; configure sensor 8 as second bed temp in plate - Aurarum cartrdge thermistor
M308 S1 P"e0temp" A"Nozzle-1" Y"thermistor" T100000 B4725 C7.060000e-8  ; configure sensor 1 as thermistor on pin e0temp - E3D cartrdge thermistor
...
M307 H0 R0.768 K0.418:0.000 D3.00 E1.35 S0.80 B0 ; Set PID for bed
...
;water pump
M950 F1 C"!fan1+^exp.e4stop" Q25000                             ; create fan 1 (water pump) on inverted pin fan1 and set its frequency.  Set RPM to exp.e4stop with pullup enabled
...
M950 F2 C"!fan2+^exp.e3stop" Q25000                              ; create radiator fan 2 on pin fan2 and set its frequency. Set RPM to e3_stop
M308 S3 P"e1temp" Y"thermistor" A"Water temp" T10000 B3950  	; Configure Water temp sensor
M106 P2 C"Radiator Fan" H3 L0.1 X1 B1.2  T40:45    	; set fan 4 value, turn on at 10% if the water temperature reaches 35C, and increase to full speed gradually as the temperature rises to 38C
...
echo "Tool in use = " , state.currentTool
