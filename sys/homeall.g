...
G1 H1 X{-(move.axes[0].max+50)} Y{-(move.axes[1].max+50)} F1800 ; move quickly to X and Y axis endstops and stop there (first pass)
if result != 0
	abort "Print cancelled due error during fast homing"
G1 H2 X5 Y5 F1000       ; go back a few mm
G1 H1 X{-(move.axes[0].max+50)} Y{-(move.axes[1].max+50)} F360  ; move slowly to X and Y axis endstops once more (second pass)
if result != 0
