...
G1 H1 X{-(move.axes[0].max+50)} F1200 ; move quickly to X axis endstop and stop there (first pass)	
if result != 0
	abort "Error duing fast homing X axis - process cancelled"
G1 H2 X5 F1200    ; go back a few mm
G1 H1 X{-(move.axes[0].max+50)} F360  ; move slowly to X axis endstop once more (second pass)
if result != 0
