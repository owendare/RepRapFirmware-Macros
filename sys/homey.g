...
G1 H1 Y{-(move.axes[1].max+50)} F1200 ; move quickly to Y axis endstop and stop there (first pass)
if result != 0
	abort "Error duing fast homing Y axis- process cancelled"
G1 H2 Y5 F1200    ; go back a few mm
G1 H1 Y{-(move.axes[1].max+50)} F360  ; move slowly to Y axis endstop once more (second pass)
if result != 0
